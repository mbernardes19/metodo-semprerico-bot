// type QueryFunction = (sql: string) => Promise<[RowDataPacket, FieldPacket[]]>;
import { RowDataPacket } from 'mysql2/promise';

export type Id = number|string

export default interface GenericDao<T extends RowDataPacket> {
  save(item: T): Promise<Id>
  delete(id: Id): Promise<void>
  update(id: Id, column: string, value: string|number): Promise<void>
  get(id: Id): Promise<T>
...
//     } = usuario;
//       await this._query(`insert into Usuario (id, nome_completo, telefone, email, forma_de_pagamento, status_assinatura) values ('${idTelegram}', '${nomeCompleto}', '${telefone}', '${email}', '${formaDePagamento}', '${statusAssinatura}')`);
//   }

//   async limparBancoDeDados() {
//     await this._query('truncate table Usuario');
//     await this._query('truncate table EmailBloqueado');
//   };

//   async adicionarEmEmailsBloqueados (email: string) {
//     const [query] = await this._conexao.query<EmailBloqueadoPacket[]>('select * from EmailBloqueado', []);
//     const emailsBloqueados = query;
//     const emailsIguais = emailsBloqueados.filter((emailBloqueado) => emailBloqueado.email === email);
//     if (emailsIguais.length === 0) {
//       await this._query(`insert into EmailBloqueado values ('${email}')`);
//     }
//   };

//   const adicionarEmNumerosBloqueados = async (numero: string) => {
