...
class Composer {
  ...
  static dispatch(routeFn, handlers) {
    return typeof routeFn === 'function' ? Composer.lazy(ctx => Promise.resolve(routeFn(ctx)).then(value => handlers[value])) : handlers[routeFn];
  }
  ...
  static entity(predicate, ...fns) {
    ...
    return Composer.optional(ctx => {
      ...
      return entities && entities.some(entity => predicate(entity, text.substring(entity.offset, entity.offset + entity.length), ctx));
    }, ...fns);
  }
  ...
  static match(triggers, ...fns) {
    return Composer.optional(ctx => {
      const text = ctx.message && (ctx.message.caption || ctx.message.text) || ctx.callbackQuery && ctx.callbackQuery.data || ctx.inlineQuery && ctx.inlineQuery.query;
  ...
  static command(command, ...fns) {
    ...
    return Composer.mount('text', Composer.lazy(ctx => {
      const groupCommands = ctx.me && (ctx.chat.type === 'group' || ctx.chat.type === 'supergroup') ? commands.map(command => `${command}@${ctx.me}`) : [];
      return Composer.entity(({
        offset,
        type
      }, value) => offset === 0 && type === 'bot_command' && (commands.includes(value) || groupCommands.includes(value)), ...fns);
    }));
  }
  ...
  static memberStatus(status, ...fns) {
    ...
    return Composer.optional(ctx => ctx.message && ctx.getChatMember(ctx.message.from.id).then(member => member && statuses.includes(member.status)), ...fns);
  }
  ...
  static unwrap(handler) {
    ...
    return typeof handler.middleware === 'function' ? handler.middleware() : handler;
  }
